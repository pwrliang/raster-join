...
inline QSize getResolution(const QPointF &leftBottom, const QPointF &rightTop, int maxCellsPerDimension) {
    ...
    QSize
    size(std::abs(ceil(diff.x() / cellSize)), std::abs(ceil(diff.y() / cellSize)));
    return size;
}


// returns true if point is to be used, and the transformed point is stored
inline bool transformPoint(PointF latlon, PointF &transformed, const BoundD &bound) {
    if (latlon.x() < bound.min_lat || latlon.x() > bound.max_lat) {
        return false;
    }

    if (latlon.y() < bound.min_lon || latlon.y() > bound.max_lon) {
        return false;
    }

    QPointF center = geo2world(QPointF((bound.min_lat + bound.max_lat) / 2, (bound.min_lon + bound.max_lon) / 2));
