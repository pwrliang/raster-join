...
QPolygonF getPolygonAtHeight(double myHeight, QPolygonF bottomPolygon, double bottomHeight, QPolygonF topPolygon,
                             double topHeight) {
...
//    auto maxlon = bound.maxy;
    ...
    if (minlon < -180 || maxlat > 180) {
        printf("Error lon range: %.6f, %.6f\n", minlat, maxlat);
        exit(1);
    }
